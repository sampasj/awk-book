...
### 年齢チェックの人数合計確認。（本項オリジナル）
最終行の263を$2にするため、フィールドセパレータFSを" "と"\t"にしている。既定では行先頭のスペースやタブは無視されるため、263は１番目($1)と見做される。
```
...
第2項をソートキーにしているため2項めの解釈がうまくいっていないレコードがあるためにおかしな表示順になっているのかもしれない。  
数字を1項めにしてソートするcharfreq-kai を作成した。
```
...
$
```
これならちゃんと出現文字数の降順になっている。  
書籍の出力サンプル。最後の数字229は明らかにおかしいのに、「黑」については言及があるのに229は無視されている。これが盲点というやつか？  
```
ア      1
ケ      1
サ      1
